...
class CZIDataset(Dataset):
    def __init__(self,csv_path,img_path_column, channel_column, out_key, spatial_dims= 3, scene_column='scene', time_start_column='start', time_stop_column='stop',time_step_column='step', transform: Optional[Callable] = None):
        ...
        if scenes != -1:
            ...
            for scene in scenes:
                if scene not in img.scenes:
                    raise ValueError(f'For image {row[self.img_path_column]} unable to find scene `{scene}`, available scenes are {img.scenes}')
        else:
            scenes = img.scenes
        return scenes
        ...
        for row in df.itertuples():
            ...
            for scene in scenes:
                for timepoint in timepoints:
                    img_data.append({'img':img,'dimension_order_out':'ZYX'[-self.spatial_dims:] ,'C':row[self.channel_column], 'scene': scene, 'T':timepoint, 'original_path': row[self.img_path_column]})
    ...
    def _transform(self, index: int):
        ...
        return {self.out_key:apply_transform(self.transform, data_i) if self.transform is not None else data_i,
                f"{self.out_key}_meta_dict":{'filename_or_obj': original_path.replace(".", self._metadata_to_str(img_data))}}
    
    def __len__(self):
        return self.n_files


def make_CZI_dataloader(csv_path,img_path_column, channel_column,  out_key, spatial_dims=3, scene_column='scene', time_start_column='start', time_stop_column='stop',time_step_column='step', transforms=None, **dataloader_kwargs):
    if isinstance(transforms, (list, tuple, ListConfig)):
        transforms = Compose(transforms)
    dataset = CZIDataset(csv_path, img_path_column, channel_column, out_key, spatial_dims, scene_column=scene_column, time_start_column=time_start_column, time_stop_column=time_stop_column,time_step_column=time_step_column, transform=transforms)
