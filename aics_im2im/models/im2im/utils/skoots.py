...
from monai.transforms import Transform, RandomizableTransform, Flip
from skimage.segmentation import find_boundaries
import numpy as np
from scipy.ndimage import find_objects
from omegaconf import ListConfig
from skimage.morphology import ball, erosion, skeletonize, dilation, remove_small_objects, disk
...
class SkootsPreprocessd(Transform):
    ...
    def __init__(
        self, label_keys: Union[Sequence[str], str], kernel_size: int= 3, thin:int= 5, dim: int = 3, anisotropy: float =2.6, allow_missing_keys: bool = False
    ):
        """
        Parameters
        ----------
    ...
    def topology_preserving_thinning(self, bw, min_size =100):
        '''
        Topology-preserving thinning of a binary image. Use skeleton to bridge gaps created by erosion.
        '''
    ...
    def smooth_embedding(self, embedding):
        '''
        Smooths embedding by convolving with a mean kernel, exluding non-object pixels
        '''
        kernel = np.ones((self.kernel_size, self.kernel_size, self.kernel_size))/(self.kernel_size **3)
    ...
    def embed_from_skel(self, skel, iseg):
        ...
        skel_boundary = torch.from_numpy(find_boundaries(skel.numpy(), mode = 'inner')) * skel # propagate labels
        for i in np.unique(iseg)[1:]:
            ...
            point_embeddings = self._get_point_embeddings(object_points.mul(self.anisotropy), skel_points.mul(self.anisotropy))
        ...
        coordinates = torch.stack(torch.meshgrid(torch.linspace(0, anisotropic_shape[0]-1, iseg.shape[0]),
...
class SkootsCluster:
    ...
    '''
    def __init__(self, anisotropy:float = 2.6, skel_threshold:float = 0, semantic_threshold:float=0, min_size:int=1000, distance_threshold:int = 100):
    ...
    def kd_clustering(self, embed_z, embed_y, embed_x, skel):
        ...
        dist_to_closest_skel, closest_skel_point_to_embedding = self._get_point_embeddings( embed_points.T, skel_points)
        embedding_labels = skel[closest_skel_point_to_embedding[0], closest_skel_point_to_embedding[1], closest_skel_point_to_embedding[2]]
    ...
    def __call__(self, image):
        ...
        anisotropic_shape = torch.as_tensor(semantic.shape).mul(torch.as_tensor([self.anisotropy, 1, 1]))
        coordinates = torch.stack(torch.meshgrid(torch.linspace(0, anisotropic_shape[0]-1, semantic.shape[0]),
        ...
        skel = apply_hysteresis_threshold(skel, high=self.skel_threshold, low = self.skel_threshold-1)
        skel = label(skel)
