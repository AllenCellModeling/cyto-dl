...
from monai.networks.blocks import UnetOutBlock, UnetBasicBlock
...
class Patchify(torch.nn.Module):
    ...
    def forward(self, img):
        ...
        tokens = torch.nn.functional.conv3d(img, weight = self.resample_weight(patch_size), stride= patch_size)
...
class MAE_Decoder(torch.nn.Module):
    def __init__(self,
        ...
        self.patch2img = Rearrange('(n_patch_z n_patch_y n_patch_x) b (c patch_size_z patch_size_y patch_size_x) ->  b c (n_patch_z patch_size_z) (n_patch_y patch_size_y) (n_patch_x patch_size_x)',
            n_patch_z = num_patches,
            n_patch_y = num_patches, 
            n_patch_x = num_patches, 
            patch_size_z =base_patch_size[0], patch_size_y=base_patch_size[1], patch_size_x = base_patch_size[2])
    ...
    def forward(self, features, backward_indexes, patch_size):
        ...
        img = torch.nn.functional.interpolate(img,tuple(torch.as_tensor( patch_size) * self.num_patches), mode='trilinear')
        img = self.conv(img)

        mask = self.patch2img(mask)
        mask = torch.nn.functional.interpolate(mask, tuple(torch.as_tensor(patch_size) * self.num_patches), mode='nearest')
...
class MAE_ViT(torch.nn.Module):
    def __init__(self,
        ...
        self.encoder = MAE_Encoder(num_patches, base_patch_size, emb_dim, encoder_layer, encoder_head, mask_ratio)
        self.decoder = MAE_Decoder(num_patches,  base_patch_size, emb_dim, decoder_layer, decoder_head)
