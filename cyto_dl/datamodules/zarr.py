...
class ZARRDataset(Dataset):
    """Dataset converting a `.csv` file listing ZARR files and some metadata into batches of single-
    ...
    def __init__(
        ...
        csv_path: Union[Path, str]
            path to csv
        img_path_column: str
            column in `csv_path` that contains path to ZARR file
        channel_column:str
            Column in `csv_path` that contains which channel to extract from ZARR file. Should be an integer.
        out_key:str
        ...
        scene_column:str="scene",
            Column in `csv_path` that contains scenes to extract from ZARR file. If not specified, all scenes will
            be extracted. If multiple scenes are specified, they should be separated by a comma (e.g. `scene1,scene2`)
        time_start_column:str="start"
            Column in `csv_path` specifying which timepoint in timelapse ZARR to start extracting. If any of `start_column`, `stop_column`, or `step_column`
            are not specified, all timepoints are extracted.
        time_stop_column:str="stop"
            Column in `csv_path` specifying which timepoint in timelapse ZARR to stop extracting. If any of `start_column`, `stop_column`, or `step_column`
            are not specified, all timepoints are extracted.
        time_step_column:str="step"
    ...
    def get_per_file_args(self, df):
        ...
        for row in df.itertuples():
            ...
            img = BioImage(row[self.img_path_column], reader=bioio_ome_zarr.Reader)
        ...
        return {
            self.out_key: MetaTensor(
                ...
                meta={
                    ...
                    "filename_or_obj": original_path.replace(".ome.zarr", self._metadata_to_str(img_data)).replace(':','-')
                },
            )
        }
...
def make_ZARR_dataloader(
    ...
    **dataloader_kwargs,
    ...
    """Function to create a ZARR Dataset. Currently, this dataset is only useful during prediction
