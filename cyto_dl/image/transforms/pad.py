...
class PadZd(Transform):
    """Transform for randomly padding top or bottom of crop by repeating first/last slice. Only applied if no segmentation is present in first/last slice"""
    def __init__(
        self,
        image_key: str,
    ...
    def __call__(self, img_dict):
        ...
        if segmentation.shape[0] > 1 and self.segmentation_ch is None:
            raise ValueError("segmentation_ch must be specified if segmentation has more than one channel")
