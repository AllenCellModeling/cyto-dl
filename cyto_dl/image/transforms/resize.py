...
class Resize(Transform):
    ...
    def __call__(self, img):
        out_size = list(
            map(
                round,
                np.asarray(img.shape[-self.spatial_dims :]) * self.scale_factor,
            )
        )
...
class Resized(Transform):
    ...
    def __init__(
        ...
        self.resizer = Resize(
            scale_factor, spatial_dims, mode, align_corners, recompute_scale_factor, antialias
        )

    def __call__(self, img):
        for key in self.keys:
            if key in img.keys():
                img[key] = self.resizer(img[key])
