...
class DiffusionAutoEncoder(BaseModel):
    ...
    def model_step(self, stage, batch, batch_idx):
        ...
        if (self.trainer.current_epoch % self.hparams.save_images_every_n_epochs) == batch_idx == 0:
    ...
    def latent_walk(self, cond, save_name):
        ...
        with torch.no_grad():
            ...
            for _ in tqdm.tqdm(range(int(self.hparams.n_noise_samples)), desc='Sampling'):
                ...
                noise = torch.stack([torch.randn([1] + self.hparams.image_shape, device=self.device)] * cond.shape[0])
    ...
    def _generate_and_re_embed(self, z, batch_idx):
        '''check cycle consistency between latent features extracted from a real image
        and the latent features extracted from an image generated by those latent features.'''
