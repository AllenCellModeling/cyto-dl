...
class NormBatchNorm(EquivariantModule):
    def __init__(
        self, in_type: FieldType, eps: float = 1e-05, momentum: float = 0.1, affine: bool = True
    ):
        r"""
        ...
        for r in in_type.representations:
            ...
            ), f'Error! Representation "{r.name}" does not support "norm" non-linearity'
            # Norm batch-normalization assumes the fields to have mean 0. This is true as long as it doesn't contain
            # the trivial representation
            for irr in r.irreps:
                assert not in_type.fibergroup._irreps[
                    irr
                ].is_trivial(), f"Input type contains trivial representation '{irr}'"
        ...
        for s, contiguous in self._contiguous.items():
            ...
            running_var = torch.ones((1, self._nfields[s], 1, 1, 1, 1), dtype=torch.float)
            self.register_buffer(f"{s}_running_var", running_var)

            if self.affine:
                weight = Parameter(
                    torch.ones((1, self._nfields[s], 1, 1, 1, 1)), requires_grad=True
                )
    ...
    def forward(self, input: GeometricTensor) -> GeometricTensor:
        ...
        if self.training:
            ...
            for s in self._order:
                ...
                if self._contiguous[s]:
                    ...
                    # compute the norm of each field by summing the squares
                    norms = (
                        n[:, indices, :, :].view(b, -1, s, z, h, w).sum(dim=2, keepdim=False)
                    )  # .sqrt()
                ...
                vars = norms.mean(dim=1).view(1, -1, 1, 1, 1, 1) / s
                vars *= correction
        ...
        for s in self._order:
            ...
            multipliers = multipliers.expand(b, -1, s, z, h, w).reshape(b, -1, z, h, w)
