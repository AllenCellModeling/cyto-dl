...
class HieraEncoder(torch.nn.Module):
    ...
    def forward(self, img):
        ...
        return mask_unit_embeddings, mask, forward_indexes, backward_indexes
...
class HieraMAE(torch.nn.Module):
    ...
    def forward(self, img):
        features, mask, forward_indexes, backward_indexes = self.encoder(img)
...
class HieraSeg(torch.nn.Module):
    ...
    def forward(self, img):
        features, _, _, _ = self.encoder(img)
        features = rearrange(features, "b t c -> t b c")
