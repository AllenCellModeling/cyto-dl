...
class PatchifyConv(torch.nn.Module):
    ...
    def get_mask(self, img, n_visible_patches, num_patches):
        B = img.shape[0]
        indexes = [random_indexes(num_patches, device=img.device) for _ in range(B)]
        ...
        mask = torch.zeros(num_patches, B, 1, device=img.device, dtype=torch.uint8)
    ...
    def forward(self, img, mask_ratio=0.75, n_visible_patches=None, task=None):
        mask = torch.ones_like(img)
        forward_indexes, backward_indexes = None, None
        if mask_ratio > 0:
            # generate mask
            num_patches = np.prod(self.n_patches)
            n_visible_patches = n_visible_patches or int(num_patches * (1 - mask_ratio))
        ...
        pos_emb = rearrange(self.pos_embedding, "t b c -> b c t")
        pos_emb = torch.nn.functional.interpolate(pos_emb, tokens.shape[0], mode="linear")
        pos_emb = rearrange(pos_emb, "b c t -> t b c")
