...
class Patchify(torch.nn.Module):
    ...
    def __init__(
        ...
        use_task_embed: bool
            Whether to add a learnable task embedding to the tokens
        """
        super().__init__()
        ...
        if use_task_embed:
            self.task_embedding = torch.nn.Parameter(torch.zeros(1, 1, emb_dim))
        self._init_weight()

    def _init_weight(self):
        trunc_normal_(self.pos_embedding, std=0.02)
        if self.use_task_embed:
            trunc_normal_(self.task_embedding, std=0.02)
