...
class Seg_ViT(torch.nn.Module):
    ...
    def __init__(
        ...
        if encoder_ckpt is not None:
            ...
            enc_state_dict = {k.replace("backbone.encoder.", ""): v for k, v in model['state_dict'].items() if 'encoder' in k}
        ...
        self.decoder = SupperresDecoder(
            num_patches, base_patch_size, emb_dim, decoder_layer, n_decoder_filters, out_channels, upsample_factor
        )
