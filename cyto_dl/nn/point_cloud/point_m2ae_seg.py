...
class H_Encoder_seg(nn.Module):
    ...
    def forward(self, neighborhoods, centers, idxs, eval=False):
        ...
        for i in range(len(x_vis_list)):
            x_vis_list[i] = (
                self.encoder_norms[i](x_vis_list[i]).transpose(-1, -2).contiguous()
            )
        return x_vis_list
...
class Point_M2AE(nn.Module):
        ...
        for i in range(3):
            self.propagations.append(
                PointNetFeaturePropagation_(
                    in_channel=self.encoder_dims[i] + 3, mlp=[self.trans_dim * 4, 1024]
                )
            )
    ...
    def forward(self, pts, cls_label=None):
        # pts = pts.transpose(-1, -2).contiguous()  # make B 3 N
        B, N, C = pts.shape
        ...
        x = torch.cat(
            (x_vis_list[0], x_vis_list[1], x_vis_list[2]), dim=1
        )  # 96 + 192 + 384
