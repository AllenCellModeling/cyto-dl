...
class H_Encoder(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        for i in range(len(self.encoder_dims)):
            if i == 0:
                ...
                self.token_embed.append(
                    Token_Embed(
                        in_c=self.encoder_dims[i - 1], out_c=self.encoder_dims[i]
                    )
                )
        ...
        dpr = [
            x.item()
            for x in torch.linspace(0, drop_path_rate, sum(self.encoder_depths))
        ]
        for i in range(len(self.encoder_depths)):
            self.encoder_blocks.append(
                Encoder_Block(
                    ...
                    drop_path_rate=dpr[
                        depth_count : depth_count + self.encoder_depths[i]
                    ],
                    num_heads=num_heads,
                )
            )
    ...
    def forward(self, neighborhoods, centers, idxs, eval=False):
        ...
        for i in range(len(neighborhoods) - 1, 0, -1):
            ...
            masked_pos = (
                torch.ones(b * centers[i - 1].shape[1])
                .cuda()
                .scatter(0, idx_masked, 0)
                .bool()
            )
        ...
        for i in range(len(centers)):
            ...
            # intermediate layers, conduct token merging
                ...
                x_vis_neighborhoods = x_vis.reshape(b * g1, -1)[idxs[i], :].reshape(
                    b, g2, k2, -1
                )
            ...
            if self.local_radius[i] > 0:
                mask_radius, xyz_dist = self.local_att_mask(
                    masked_center, self.local_radius[i], xyz_dist
                )
...
class Point_M2AE(nn.Module):
    def __init__(
        ...
        self.decoder_up_blocks = decoder_up_blocks
        ...
        dpr = [
            x.item()
            for x in torch.linspace(0, drop_path_rate, sum(self.decoder_depths))
        ]
        for i in range(0, len(self.decoder_dims)):
            ...
            self.h_decoder.append(
                Decoder_Block(
                    ...
                    drop_path_rate=dpr[
                        depth_count : depth_count + self.decoder_depths[i]
                    ],
                    num_heads=num_heads,
                )
            )
    ...
    def forward(self, pts, eval=False, **kwargs):
        ...
        for i in range(len(self.decoder_dims)):
            ...
            if i == 0:
                ...
                B, _, C = x_full.shape
                center_0 = torch.cat(
                    (center[~mask].reshape(B, -1, 3), center[mask].reshape(B, -1, 3)),
                    dim=1,
                )

                pos_emd_vis = self.decoder_pos_embeds[i](center[~mask]).reshape(
                    B, -1, C
                )
                pos_emd_mask = self.decoder_pos_embeds[i](center[mask]).reshape(
                    B, -1, C
                )
                ...
                if i == 1:
                    ...
                    x_full = self.token_prop[i - 1](
                        center, centers[i - 1], x_full_en, x_full
                    )
