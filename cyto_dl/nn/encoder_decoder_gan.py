...
class EncoderDecoderGAN(torch.nn.Module):
    def __init__(self, encoder, decoder, encoder_ckpt=None, decoder_ckpt=None, train_encoder=True, train_decoder=True, latent_norm = None):
        super().__init__()
    ...
    def _init_model(self, model, ckpt, train):
        if ckpt is not None:
            ...
            state_dict = {k.replace("backbone.", ""): v for k, v in state_dict.items() if 'net.' in k or 'final' in k}
