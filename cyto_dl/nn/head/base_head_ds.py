...
class BaseHead_DS(ABC, torch.nn.Module):
    ...
    def run_head(
        ...
        loss = None
        #print('printing y_hat shape')
        #print(y_hat.shape)
        if stage != "predict":
            if len(y_hat.shape) > 5:
                y_hat_main = torch.unbind(y_hat, 1)[0]
                y_hat_side1 = torch.unbind(y_hat, 1)[1]
                #y_hat_side2 = torch.unbind(y_hat, 1)[2]
                #loss = self._calculate_loss(y_hat_main, batch[self.head_name]) + self._calculate_loss(y_hat_side1, batch[self.head_name]) + self._calculate_loss(y_hat_side2, batch[self.head_name])
                loss = self._calculate_loss(y_hat_main, batch[self.head_name]) + self._calculate_loss(y_hat_side1, batch[self.head_name])
            else:
